...
QString CheckpointsPlugin::name(){
    return "Checkpoints";
}

QString CheckpointsPlugin::pluginVersion() {
    return "0.0.1";
}

QString CheckpointsPlugin::pluginInterfaceVersion(){
    return "1.0.1";
}

QString CheckpointsPlugin::description(){
    return "A simple plugin to verify that preset sequences exist in the dlt log.";
}

QString CheckpointsPlugin::error() {
    return errorText;
}
...
QStringList CheckpointsPlugin::infoConfig(){
    return QStringList {};
}
...
void CheckpointsPlugin::initFileFinish(){
    form->displayInfo("Done");
}
...
bool CheckpointsPlugin::initConnections(QStringList)
{
    return false;
}

bool CheckpointsPlugin::controlMsg(int, QDltMsg &)
{
    return false;
}

bool CheckpointsPlugin::stateChanged(int, QDltConnection::QDltConnectionState, QString)
{
    return false;
}

bool CheckpointsPlugin::autoscrollStateChanged(bool)
{
    return false;
}
...
void CheckpointsPlugin::addSequence(Sequence *s)
{
    ...
    std::for_each(s->sequence.begin(), s->sequence.end(), [&](const QString& payload){
        form->addSublistItem(payload,seq_item);
    });
}
...
void CheckpointsPlugin::updateSequenceByName(Sequence *s)
{
    ...
    std::for_each(s->sequence.begin(), s->sequence.end(), [&](const QString& payload){
        form->addSublistItem(payload,new_item);
    });
}
...
void CheckpointsPlugin::exportSequences()
{
    ...
    QString file_name = QFileDialog::getSaveFileName(nullptr, "Save File", "", "*.json");
...
{
    QString file_name = QFileDialog::getOpenFileName(nullptr, "Open a file", "directoryToOpen", "*.json");
    if (file_name.isEmpty()) {
    ...
    // Step 5: Iterate through each sequence object and extract "name" and "payloads"
...
void CheckpointsPlugin::clear()
{
    sequences_->sequences.clear();
}
...
\
    ...
    matcher.setCaseSentivity(caseSensitivity);

    matcher.setHeaderSearchEnabled(false);
    matcher.setPayloadSearchEnabled(true);

    for (long int searchLine = 0; searchLine < qFile->sizeFilter(); ++searchLine) {
        ...
        if(payload.contains(search_msg)){
            form->displayInfo("message " + search_msg + " was found" + " index: " + QString::number(msg.getIndex()));
            foundMsg = searchLine;
            break;
        }
    }
...
void CheckpointsPlugin::jumpToMsg(int index)
{
    dltControl->jumpToMsg(index);
}
