...
chpsForm::chpsForm(CheckpointsPlugin* plugin, QWidget* parent)
    : QWidget(parent), ui(new Ui::CheckpointsForm), plugin_(plugin), dialog_(new ChpDialogForm(plugin_, parent))
{
    ui->setupUi(this);
    ...
    QObject::connect(ui->chpsResults, SIGNAL(itemDoubleClicked(QListWidgetItem*)), this,
    ...
    connect(deleteAction, &QAction::triggered, this,
            [&]() { delete ui->chpsResults->takeTopLevelItem(ui->chpsResults->indexOfTopLevelItem(selectedItem)); });
}
...
void chpsForm::displayInfo(QString s)
{
...
void chpsForm::on_exportBtn_clicked()
{
    plugin_->exportSequences();
}
...
void chpsForm::on_importBtn_clicked()
{
    plugin_->importSequences();
}
...
void chpsForm::processTopItem(QTreeWidgetItem* topItem, bool& seqFailed)
{
    ...
    topItem->setIcon(0, seqFailed ? QIcon(":/png/icon-bad") : QIcon(":/png/icon-ok"));
}
...
void chpsForm::on_verifyBtn_clicked()
{
    ...
    {
        bool* seqFailed = new bool(false); // Pointer to capture result per thread
        QTreeWidgetItem* topItem = ui->chpsResults->topLevelItem(i);

        // Launch the task asynchronously and monitor with watcher
        QFuture<void> future = QtConcurrent::run(this, &chpsForm::processTopItem, topItem, std::ref(*seqFailed));
        QObject::connect(watcher, &QFutureWatcher<void>::finished, this, [=, &completedTasks]() mutable {
            if (++completedTasks == seq_count)
            {
                // All tasks are completed
                // Handle any final UI updates or cleanup here
                watcher->deleteLater();
            }
        });

        watcher->setFuture(future); // Update watcher with the new future
    }
}

void chpsForm::contextMenuEvent(QContextMenuEvent* event)
{
    selectedItem = ui->chpsResults->itemAt(ui->chpsResults->viewport()->mapFromGlobal(event->globalPos()));
